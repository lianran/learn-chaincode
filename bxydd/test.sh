...
peer chaincode query -n ${mycc} -c "{\"Args\": [\"balance\", \"a\"]}"
echo "get teh balance of b"
peer chaincode query -n ${mycc} -c "{\"Args\": [\"balance\", \"b\"]}"
echo "get the history of a"
peer chaincode query -n ${mycc} -c "{\"Args\": [\"history\", \"a\",\"233\"]}"
...
peer chaincode query -n ${mycc} -c "{\"Args\": [\"history\", \"a\",\"233\"]}"
peer chaincode query -n ${mycc} -c "{\"Args\": [\"history\", \"b\",\"233\"]}"
